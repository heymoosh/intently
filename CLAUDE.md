...
**Chat-driven edits — worktree first.** Any conversation ask that touches a tracked file gets a fresh worktree off `main` (`git worktree add ~/wt/<slug> -b chat/<slug> main`) BEFORE writing. The main checkout NEVER changes branch — it stays on `main`. Edits, commits, and PRs happen in the worktree's directory; when the PR merges, remove the worktree (`git worktree remove ~/wt/<slug>`). This keeps Muxin's working dir always-clean-on-main, eliminates the "wait, what branch am I on?" failure mode, and matches the `intently-track` parallel-track pattern. Pre-commit hook enforces no commits on `main`.
...
**Destructive ops require per-call confirmation, even when broadly authorized.** "Clean it up," "do whatever's needed," "delete the cruft" are NOT blanket authorization for individual destructive ops. Each one needs an explicit go before running: `git worktree remove --force` (deletes directory contents instantly, no Trash), `git stash drop`, `git branch -D`, `git push --force`, `git reset --hard`, `git clean -f`, `rm -rf`, `supabase db push/reset`, `gh pr close`, `gh repo delete`. **Pre-flight check: before deleting any directory, list its contents** — especially worktrees (often hold uncommitted user work). Prefer reversible alternatives (move to Trash, soft-reset, stash with description). Harness-level gating in `.claude/settings.json` § permissions.ask backs this up as a second line of defense. Reason: 2026-04-25, force-removed `~/.claude/worktrees/video-demo/` while doing "cleanup" → lost user's hours of untracked work (script + assets). Never again.
