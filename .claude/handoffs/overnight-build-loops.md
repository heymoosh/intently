...
8. **Stacked-PR survival on parent merge.** Third failure mode from 2026-04-25 night, distinct from silent-iter and Mac-sleep: when iter 1's PR (#97) squash-merged with `--delete-branch`, GitHub auto-closed the dependent stacked PRs (#98 iter 3, #99 iter 2) because their base branch evaporated. Manual recovery required — opening fresh PRs from the orphaned remote branches against main (#102, #103). Three options to evaluate when rewriting the loop brief: (a) retarget dependent PRs to `main` automatically before parent merges, (b) skip `--delete-branch` on parents until all dependents have merged, (c) abandon true PR stacking — instead use a single rolling branch where each iter commits onto the previous, opening one PR per night that bundles all iters (simpler review, no cascade risk; downside: bisecting a single-PR-per-night is harder if one iter regresses something). Recommendation pending: option (c) is the cleanest given the loop produces independent docs anyway; option (a) is the right call if true stacking is wanted for cross-PR review chains. Decide while writing #1.
