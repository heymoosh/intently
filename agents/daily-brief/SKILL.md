...
**At session start:** List `/mnt/memory/` with the file tool. Read `/mnt/memory/yesterday-brief.md` — avoid repeating the same pacing note or highlights from yesterday.

**At session end:** Write `/mnt/memory/yesterday-brief.md` with a 2–3 sentence summary of today's brief (pacing, P1 focus, key carry-forward). Overwrite each run; only yesterday matters.
