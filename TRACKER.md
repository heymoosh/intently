...
- **Delete 3 bunk BWS entries — user-only via web UI.** BWS has three stale agent-ID secrets that the proxy doesn't read (truth is Supabase `MA_AGENT_ID_*`): `daily-review` (`1855bb28-d920-48fd-acea-b43601859e0f`), `weekly-review` (`2a7ce2a6-1fb0-41ea-a91c-b4360185c389`), `monthly-review` (`94687829-48ce-4e38-a87b-b4360185dc20`). The local `BWS_ACCESS_TOKEN` lacks delete permission; remove via vault.bitwarden.com. The surviving `ANTHROPIC_API_KEY` entry already renamed off "daily brief" to "Anthropic API Key" (2026-04-25).
