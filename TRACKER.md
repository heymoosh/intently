...
- **Codify the "start overnight" evening ritual into one command.** *(Added 2026-04-23 evening. Supersedes earlier "Promote overnight build loop to recurring via launchd 23:30" — that approach is blocked by lid-closed wake/sleep limits; see PR #45 discussion.)* Tonight's manual steps — (a) Scope Overnight Steward fires at 21:00 writing `routine-output/scope-overnight-<date>.md`, (b) Claude hand-tweaks it if needed, (c) user runs `pkill -f "caffeinate -dims"; caffeinate -dims -t <secs-to-8am> &` to replace the forever-caffeinate with a timed one, (d) user opens Claude Code and pastes `/loop 1h Read .claude/loops/overnight-build-loop.md ...`. Worked tonight (2 PRs shipped by 20:33; expect 4 more by ~05:30). Make it a shell script at `~/.intently/bin/intently-start-overnight.sh` that does (c) + prints (d) as a paste-ready command; step (a) already automated, step (b) is optional. Script computes the seconds-to-08:00 itself so the user doesn't math it. Leaves the /loop kickoff manual (session-bound loops can't be launched from cron). Discuss Friday live session — non-critical path, backlog it if hackathon time is tight.
