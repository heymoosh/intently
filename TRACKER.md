...
- **`real-app-cognition`** → `.claude/handoffs/real-app-cognition.md` — gap between shipped prototype shell (capture + display + persist + hydrate) and original UX vision (agent that knows you across sessions). 11-item leverage-ordered backlog covering context assembler, cross-day continuity, setup flow, weekly/monthly review UI, real reads in Past/Future tabs, input traces, undo. Opened 2026-04-25 post-#143. Status: active (#1 — context assembler — starting next).
...
- **Persona drift in `web/` — Maya → Sam pass.** Deployed prototype shipped as Maya throughout (9 refs across 5 files: greeting × 2 surfaces, conversational agent, profile name + email × 2, reading byline, design-system byline, avatar initials). Sam is canon per `seed/README.md` + `evals/datasets/daily-brief/` + `app/` (Sam-locked). Pass in flight in `chat/0425-161122` — relevant for demo recording since Maya was appearing in the live URL. Originally captured as a 3pm-greeting-only cosmetic follow-up from PR #125; actual scope was broader.
