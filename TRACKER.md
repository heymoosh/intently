...
- **[Resolved 2026-04-25] Anthropic API key rotated + consolidated.** Old "daily brief" and "review agents" keys deleted in Anthropic console; replaced with a single consolidated key that runs all 6 agents. BWS and Supabase verified to hold the same value via SHA256 digest match against `cd0563ca…6c75e8`. The old exposed values are now dead at the API level regardless of where they leaked.
- **Clean up BWS entries.** (1) BWS has three malformed agent-ID secrets (`daily-review`, `weekly-review`, `monthly-review`) created during provisioning with whitespace contamination + IDs from a *different* workspace than the proxy uses. Source of truth for agent IDs is now Supabase secrets (`MA_AGENT_ID_*`); delete the bunk BWS entries. (2) The surviving BWS `ANTHROPIC_API_KEY` entry is still labeled "daily brief" — rename it (e.g. `ANTHROPIC_API_KEY` or `ANTHROPIC_API_KEY_INTENTLY`) to reflect that it's the single consolidated key for all 6 agents, not just daily-brief. User-only.
