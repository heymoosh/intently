...
- **update-tracker + setup MA agents — IN SCOPE (no longer deferred, 2026-04-25).** Both have full `ma-agent-config.json` authored (sonnet-4-6, ~9–13K system prompts). Provision via `scripts/provision-ma-agents.ts`, set `MA_AGENT_ID_SETUP` + `MA_AGENT_ID_UPDATE_TRACKER` in Supabase secrets, redeploy ma-proxy. SKILL.md files banner-marked as superseded by JSON.
